...
const ProductsData = ({ item }: ItemProps) => {
  ...
  const startArray = Array.from({ length: item?.rating || 5 }, (_, index) => (
  ...
  return (
    <div className="w-full rounded-lg overflow-hidden">
      <div>
        <Link href={{ pathname: "/product", query: { id: item?.id || item?._id } }}>
          <div className="w-full h-96 group overflow-hidden relative">
            <Image
              src={item?.image || (item?.imageData ? `data:image/jpeg;base64,${item.imageData}` : "/next.svg")}
              alt="product image"
              width={500}
              height={500}
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() =>
                ...
                toast.success(
                  `${(item?.name || item?.title || "").substring(0, 15)} added successfully!`
                )
              }
