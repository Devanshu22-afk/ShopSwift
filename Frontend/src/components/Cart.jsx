  ...
  useEffect(() => {
    const fetchImagesAndUpdateCart = async () => {
      ...
      try {
        ...
        // Merge cart items with product data to ensure imageData is available
        const updatedCartItems = cart.map(cartItem => {
          const product = products.find(p => p.id === cartItem.id);
          if (product) {
            // Merge cart item (with quantity) with product data (with imageData)
        ...
        // Fallback to cart items as-is if API fails
        setCartItems(cart);
      }
    };
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        ...
        // Don't send imageFile since we're only updating stock quantity
        // The backend will keep the existing image if imageFile is null/empty
        cartProduct.append("imageFile", "");
        cartProduct.append(
          "product",
