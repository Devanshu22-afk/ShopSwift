    ...
    const fetchOrders = async () => {
      try {
        ...
        if (error.response) {
          ...
          if (status === 401) {
            setError(errorMessage || "Please login to view your orders.");
          } else if (status === 404) {
            setError(errorMessage || "User information not found. Please try logging in again.");
          } else if (status === 400) {
            setError(errorMessage || "Your account is missing email information. Please contact support or register again with an email.");
          } else if (status === 500) {
            setError(errorMessage || "Server error. Please try again later.");
          } else {
            setError(errorMessage || `Failed to fetch orders (Error ${status}). Please try again later.`);
          }
